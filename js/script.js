...
const finishBuy = () => {
  const finalCart = cart.map(item => {
    return {
      id: item.id,
      name: pizzaJson[item.id - 1].name,
      qt: item.qt,
      size: item.size
    }
  })

  const string_orders = finalCart.reduce((prev, current) => {
    return `${prev} ${current.qt} ${current.qt > 1 ? 'pizzas' : 'pizza'} de ${
      current.name
    } tamanho ${current.size}\n`
  }, '')

  window.open(
    `https://wa.me/5588997173677?text=${`Olá, Gostaride de pedir: ${string_orders}`}`
  )
}
